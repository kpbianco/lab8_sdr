...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    float time_spent = 1; 
    const float f_clk_hz = 125e6f;                             // AXI clock in Hz
    unsigned int ticks = stop_time - start_time;               // elapsed timer ticks
    bytes_transferred = 2048u * sizeof(unsigned int);          // 2048 reads * 4 bytes
    time_spent = ticks / f_clk_hz;                             // seconds
    throughput = (bytes_transferred / time_spent) / 1.0e6f;    // MB/s (decimal)
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Kian Bianco - Custom Peripheral Demonstration\n\r");
